...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://github.com/SuYi1995/ssr/tree/main/ssr_extra/bbr.sh; then
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/SuYi1995/ssr/tree/main/ssr_extra/manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://github.com/SuYi1995/ssr/tree/main/ssr_extra/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://github.com/SuYi1995/ssr/tree/main/ssr_extra/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		...
		if [[ ${bit} = "x86_64" ]]; then
			mv "jq-linux64" "jq"
			#wget --no-check-certificate "https://github.com/SuYi1995/ssr/tree/main/ssr_extra/jq-linux64" -O ${jq_file}
		else
			mv "jq-linux32" "jq"
			#wget --no-check-certificate "https://github.com/SuYi1995/ssr/tree/main/ssr_extra/jq-linux32" -O ${jq_file}
		fi
...
Install_ServerSpeeder(){
	...
	cd /root
	#借用91yun.rog的开心版锐速
	wget -N --no-check-certificate https://github.com/SuYi1995/ssr/tree/main/ssr_extra/serverspeeder.sh
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://github.com/SuYi1995/ssr/tree/main/ssr_extra/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://github.com/SuYi1995/ssr/tree/main/ssr_extra/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	...
	wget -N --no-check-certificate "https://github.com/SuYi1995/ssr/tree/main/ssr_bash/ssr.sh" && chmod +x ssr.sh
